...
def find_landmarks(
    ...
    eval_points: np.ndarray,
    basis_func: Basis,
    prominence_thresh: float = 0.05,
):
    """
    Find the landmarks of the functional data. The following are considered landmarks in functional data:
        1. Peaks: Local maxima or areas where the first derivative changes from positive to negative and the second derivative is negative.
        2. Troughs: Local minima or areas where the first derivative changes from negative to positive and the second derivative is positive.
        3. Inflection Points: Areas of highest ascent or descent or where the second derivative changes sign.

    Parameters
    ----------
    coefs : np.ndarray
        The coefficients of the functional data.
    eval_points: np.ndarray
        The points at which to evaluate the functional data.
    basis_func : Basis
        The basis object.
    prominence_thresh : float
        Minimum absolute value threshold to filter out tiny background noise peaks.

    Returns
    -------
    np.ndarray
        The landmarks of the functional data.
    """

    dt = eval_points[1] - eval_points[0]

    # Create a matrix of basis function values at each evaluation point
    x_t = basis_func.evaluate(eval_points) @ coefs
    # Find the local extrema using the first derivative
    first_derivative = basis_func.evaluate_derivative(eval_points, order=1) @ coefs
    # Find the local extrema using the second derivative
    second_derivative = basis_func.evaluate_derivative(eval_points, order=2) @ coefs

    # Pre-calculate baseline statistics to make prominence adaptive
    x_mean = np.mean(x_t)

    peaks, troughs, inflection_points = [], [], []
    for i in range(len(eval_points) - 1):
        # Linear interpolation to isolate the exact location on the grid
        weight = abs(first_derivative[i]) / (
            abs(first_derivative[i]) + abs(first_derivative[i + 1])
        )
        t_landmark = eval_points[i] + weight * dt
        x_landmark = x_t[i] + weight * (x_t[i + 1] - x_t[i])

        # Check for crossing (product between positive and negative is negative)
        if first_derivative[i] * first_derivative[i + 1] < 0:
            # If first derivative goes from positive to negative, then it is a peak
            if first_derivative[i] > 0 and first_derivative[i + 1] < 0:
                if second_derivative[i] <= 0 and (
                    x_landmark > x_mean + prominence_thresh
                ):
                    peaks.append((t_landmark, x_landmark))

            # If first derivative goes from negative to positive, then it is a trough
            elif first_derivative[i] < 0 and first_derivative[i + 1] > 0:
                if second_derivative[i] >= 0 and (
                    x_landmark < x_mean - prominence_thresh
                ):
                    troughs.append((t_landmark, x_landmark))

        # Identify inflection points (areas where the rate of change of the slope is greatest)
        if second_derivative[i] * second_derivative[i + 1] < 0:
            if abs(x_landmark) > prominence_thresh:
                inflection_points.append((t_landmark, x_landmark))

    return {"peaks": peaks, "troughs": troughs, "inflection_points": inflection_points}
